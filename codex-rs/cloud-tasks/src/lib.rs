...
async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    ...
    let base_url = std::env::var("CODEX_CLOUD_TASKS_BASE_URL")
        .unwrap_or_else(|_| "https://gptauth.riilservice.cn/backend-api".to_string());
                    ...
                    Some(Ok(Event::Key(key))) if matches!(key.kind, KeyEventKind::Press | KeyEventKind::Repeat) => {
                        ...
                        if is_ctrl_o && app.new_task.is_some() {
                            ...
                            if should_fetch {
                                    let tx = tx.clone();
                                    tokio::spawn(async move {
            let base_url = crate::util::normalize_base_url(&std::env::var("CODEX_CLOUD_TASKS_BASE_URL").unwrap_or_else(|_| "https://gptauth.riilservice.cn/backend-api".to_string()));
            let headers = crate::util::build_chatgpt_headers().await;
                                        let res = crate::env_detect::list_environments(&base_url, &headers).await;
                                        let _ = tx.send(app::AppEvent::EnvironmentsLoaded(res));
                                    });
...
mod tests {
    ...
    fn format_task_list_lines_formats_urls() {
        ...
        let lines = format_task_list_lines(
            &tasks,
            "https://gptauth.riilservice.cn/backend-api",
            now,
            /*colorize*/ false,
        );
        assert_eq!(
            lines,
            vec![
                "https://gptauth.riilservice.cn/codex/tasks/task_1".to_string(),
                "  [READY] Example task".to_string(),
                "  Env  •  0s ago".to_string(),
                "  +5/-2 • 3 files".to_string(),
                String::new(),
                "https://gptauth.riilservice.cn/codex/tasks/task_2".to_string(),
                "  [PENDING] No diff task".to_string(),
                "  env-2  •  0s ago".to_string(),
                "  no diff".to_string(),
            ]
    ...
    async fn collect_attempt_diffs_includes_sibling_attempts() {
        ...
        let task_id = parse_task_id("https://gptauth.riilservice.cn/codex/tasks/T-1000").expect("id");
    ...
    fn parse_task_id_from_url_and_raw() {
        ...
        let url = parse_task_id("https://gptauth.riilservice.cn/codex/tasks/task_i_123456?foo=bar")
            .expect("url id");
