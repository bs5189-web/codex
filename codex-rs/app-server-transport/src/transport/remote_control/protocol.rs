...
pub(super) fn normalize_remote_control_base_url(remote_control_url: &str) -> io::Result<Url> {
    ...
    let map_scheme_error = |_: ()| -> io::Error {
        io::Error::new(
            ErrorKind::InvalidInput,
            format!(
                "invalid remote control URL `{remote_control_url}`; expected HTTPS URL for chatgpt.com, chatgpt-staging.com, gptauth.ruijie.com.cn, or HTTP/HTTPS URL for localhost"
            ),
        )
    };
...
mod tests {
    ...
    fn normalize_remote_control_url_accepts_chatgpt_https_urls() {
        assert_eq!(
            normalize_remote_control_url("https://gptauth.ruijie.com.cn/backend-api")
                .expect("chatgpt.com URL should normalize"),
            RemoteControlTarget {
                websocket_url: "wss://gptauth.ruijie.com.cn/backend-api/wham/remote/control/server"
                    .to_string(),
                enroll_url:
                    "https://gptauth.ruijie.com.cn/backend-api/wham/remote/control/server/enroll"
                        .to_string(),
                refresh_url:
                    "https://gptauth.ruijie.com.cn/backend-api/wham/remote/control/server/refresh"
                        .to_string(),
                pair_url:
                    "https://gptauth.ruijie.com.cn/backend-api/wham/remote/control/server/pair"
                        .to_string(),
                pair_status_url:
